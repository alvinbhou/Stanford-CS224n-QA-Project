...
from transformers import BertForQuestionAnswering, BertConfig, BertTokenizer, RobertaConfig, RobertaForQuestionAnswering, RobertaTokenizer
...
class RobertaQA(nn.Module):
    ...
    def __init__(self, model_type='bert-base-uncased', do_cls=True):
        super().__init__()
        self.do_cls = do_cls
        self.config = RobertaConfig.from_pretrained(model_type, output_hidden_states=do_cls)
        self.model = RobertaForQuestionAnswering.from_pretrained(model_type, config=self.config)
        self.fc_cls = nn.Linear(1024, 2) if 'large' in model_type else nn.Linear(768, 2)
        self.criterion_cls = nn.CrossEntropyLoss()
...
if __name__ == "__main__":
    ...
    model = RobertaQA(do_cls=False, model_type='bert-large-uncased-whole-word-masking-finetuned-squad')
